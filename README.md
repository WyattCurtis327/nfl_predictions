...
**Auto-connect on workspace open.** `.vscode/settings.json` points Python at `.venv` and loads `.env` (profile, host, and `DATABRICKS_SERVERLESS_COMPUTE_ID=auto`). A folder-open task runs `python scripts/sync_bundle_env.py`, which syncs those values into `.databricks/.databricks.env`, sets `targets.prod.workspace.host` in `databricks.yml` (required by the VS Code extension), and updates `.databricks/bundle/prod/vscode.overrides.json`. Reload the window once after cloning; when prompted to allow automatic tasks, choose **Allow**. The Databricks extension should then attach to **Serverless** using your `.env` profile on startup (sign in via the extension if prompted).
